...
def _run_optimized(df, budget_manager, base_bid, conversion_weight):
    ...
    feature_columns = ["campaign_id", "hour", "device_type", "floor_price", "market_price"]
    ...
    hourly_stats = {}
    device_stats = {"mobile": {"clicks": 0, "conversions": 0, "impressions": 0},
    ...
    for i in range(len(df)):
        ...
        if bid >= market_price and budget_manager.can_bid(market_price):
            ...
            if hour not in hourly_stats:
                hourly_stats[hour] = {"clicks": 0, "conversions": 0, "impressions": 0}
    ...
    return _format_results(
        ...
        total_spent, budget_manager.remaining_budget,
        conversion_weight, hourly_stats, device_stats
    )
...
def _format_results(total_impressions, total_clicks, total_conversions,
                   total_spent, remaining_budget, conversion_weight,
                   hourly_stats, device_stats):
    ...
    for hour in sorted(hourly_stats.keys()):
        ...
        hour_ctr = (stats["clicks"] / stats["impressions"] * 100) if stats["impressions"] > 0 else 0
    ...
    for device, stats in device_stats.items():
        if stats["impressions"] > 0:
            ...
            device_cvr = (stats["conversions"] / stats["clicks"] * 100) if stats["clicks"] > 0 else 0
